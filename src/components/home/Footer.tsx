      ...
      <div className="container mx-auto px-8 relative z-10">
              ...
              <ul className="space-y-4 text-sm font-bold text-slate-400">
                {['Home', 'Curriculum', 'Hardwares', 'Community'].map((item) => (
                  <li key={item} className="hover:text-white transition-colors cursor-pointer flex items-center gap-2 group">
                    <div className="w-0 h-[1px] bg-monia-orange group-hover:w-4 transition-all" /> {item}
                  </li>
                ))}
              </ul>
            </div>
        ...
        <div className="pt-12 border-t border-white/5 flex flex-col md:row items-center justify-between gap-8">
          <div className="flex items-center gap-3">
             <div className="p-2 bg-monia-blue rounded-lg shadow-[0_0_20px_rgba(59,130,246,0.5)]">
                <Cpu size={24} className="text-white" />
             </div>
             <span className="text-2xl font-black tracking-tighter uppercase">MONIA <span className="text-monia-orange">LABS</span></span>
          </div>
